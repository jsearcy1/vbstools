...
the difficult problem of measuring longitudinal Vector Boson Scattering (VBS),
which is integral to  electroweak symmetry breaking (EWSB) in the Standard Model (SM).

Measuring longitudinal VBS has long been a goal of high energy colliders~\cite{SSC paper}. Without a Higgs boson the scattering amplitude of longitudinal vector bosons ($V_{L}V_{L} \rightarrow V_{L}V_{L}$) increases with center-of-mass energy until unitarity is violated~\cite{three references from paper}. The discovery of a Higgs-like boson at the
LHC~\cite{ATLAS_higgs,CMS_higgs} was the first step toward understanding these interactions, however, if this Higgs boson's couplings deviate from the SM expectation unitarity violation is still possible.
This sensitivity makes VBS a possible probe of anomalous Higgs couplings~\cite{higgs coupling paper}. In addition new many new physics scenarios predict increases in VBS~\cite{aa}, through extended Higgs sectors or
other new resonances. The potential to discover new physics
and constrain fundamental properties of the Higgs Boson makes VBS measurements very valuable~\cite{vbs paper citations + higgs triplet}. However,  Measuring VBS processes at a hadron collider is experimentally challenging,
...
universal approximator~\cite{aa}, to instead approximate an unknown continuous function.
The goal of our Neural Network is to find the best approximation using measurable quantities 
of the two truth values of \cts (one for each W boson) present in each event.  
...
linear activation to approximate the true $\cts$ distribution of each $W$ boson.
 The neural network is implemented with the Theano
